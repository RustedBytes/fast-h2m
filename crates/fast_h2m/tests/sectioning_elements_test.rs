//! Integration tests for sectioning elements test.

fn convert(html: &str) -> String {
    fast_h2m::convert(html, None)
...
fn test_nav_preserved_when_remove_navigation_disabled() {
    use fast_h2m::{ConversionOptions, PreprocessingOptions};
    ...
    let result = fast_h2m::convert(html, Some(opts))
        .map(|r| r.content.unwrap_or_default())
        .expect("conversion should succeed");
    assert!(
