...
use h2md::{ConversionOptions, TierStrategy, convert};
...
fn tier1_extracts_title_with_spaces() {
    ...
    assert_eq!(
        t1(html),
        t2(html),
        "multi-word title metadata must be byte-identical"
    );
}
...
fn tier1_extracts_meta_description() {
    ...
    assert_eq!(
        t1(html),
        t2(html),
        "meta description must be byte-identical"
    );
}

#[test]
fn tier1_extracts_meta_keywords() {
    let html = r#"<html><head><meta name="keywords" content="rust, markdown, html"></head><body><p>x</p></body></html>"#;
...
fn tier1_extracts_meta_author() {
    let html =
        r#"<html><head><meta name="author" content="Jane Doe"></head><body><p>x</p></body></html>"#;
...
fn tier1_extracts_og_title() {
    let html = r#"<html><head><meta property="og:title" content="Open Graph Title"></head><body><p>x</p></body></html>"#;
    assert_eq!(t1(html), t2(html), "og:title must be byte-identical");
}

#[test]
fn tier1_extracts_og_description() {
    let html = r#"<html><head><meta property="og:description" content="OG Description"></head><body><p>x</p></body></html>"#;
...
fn tier1_extracts_canonical_url() {
    let html = r#"<html><head><link rel="canonical" href="https://example.com/page"></head><body><p>x</p></body></html>"#;
    assert_eq!(t1(html), t2(html), "canonical link must be byte-identical");
}

#[test]
fn tier1_extracts_link_author() {
    let html = r#"<html><head><link rel="author" href="https://example.com/author"></head><body><p>x</p></body></html>"#;
...
fn tier1_extracts_base_href() {
    let html =
        r#"<html><head><base href="https://example.com/"></head><body><p>x</p></body></html>"#;
...
fn tier1_extracts_multiple_meta_tags() {
    ...
    assert_eq!(
        t1(html),
        t2(html),
        "multiple meta tags must be byte-identical"
    );
}
...
fn tier1_no_head_element_produces_no_frontmatter() {
    ...
    assert_eq!(
        t1(html),
        t2(html),
        "no-head document must be byte-identical"
    );
}
...
fn tier1_no_frontmatter_when_extract_metadata_false() {
    ...
    let out_t1 = convert(html, Some(opts_t1))
        .unwrap()
        .content
        .unwrap_or_default();
    let out_t2 = convert(html, Some(opts_t2))
        .unwrap()
        .content
        .unwrap_or_default();
...
fn tier1_yaml_quotes_colon_in_value() {
    ...
    assert_eq!(
        t1(html),
        t2(html),
        "YAML-quoted colon value must be byte-identical"
    );
}
...
fn auto_routing_with_extract_metadata_can_use_tier1() {
    ...
    let auto_out = convert(html, Some(opts_auto))
        .unwrap()
        .content
        .unwrap_or_default();
    let t2_out = convert(html, Some(opts_t2))
        .unwrap()
        .content
        .unwrap_or_default();
    assert_eq!(
        auto_out, t2_out,
        "Auto with extract_metadata must match Tier-2"
    );
    assert!(
