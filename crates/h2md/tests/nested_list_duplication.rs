...
use h2md::{ConversionOptions, NodeContent};
...
fn markdown_output_no_content_duplication() {
    let result = h2md::convert(NESTED_LIST_HTML, None).unwrap();
...
fn structure_collector_list_items_no_duplication() {
    ...
    let result = h2md::convert(NESTED_LIST_HTML, Some(options)).unwrap();
    let doc = result
        .document
